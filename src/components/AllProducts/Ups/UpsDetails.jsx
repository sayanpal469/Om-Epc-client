...
const UpsDetails = () => {
    ...
    const handelBuyClick = (id) => {
        if (!localStorage.getItem('user')) {
            ...
            localStorage.setItem('buyProduct', JSON.stringify(ups))
            navigate('/buy')
        }
    };
    ...
    return (
        <div className="container px-14 py-24 mx-auto">
            ...
            {!loading && <div className="grid lg:grid-cols-2 md:grid-cols-2 sm:grid-cols-1 space-x-5">
                <img alt="ecommerce" className="w-[80%] mx-auto object-cover object-center rounded" src={imgUrl} />
                <div className="w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0">
                    {/* <h2 className="text-sm title-font text-gray-500 tracking-widest">{brand}</h2> */}
                    <h1 className="text-gray-900 text-3xl title-font font-medium mb-1">{brand} {modelNumber} {modelName} UPS</h1>
                  ...
                  >
                    ...
                    <div className='pb-5 border-b-2 border-gray-200 collapse'>
                        ...
                        <div className='flex justify-between my-4 items-center capitalize'>
                            <h3>Model Number</h3>
                            <h3 className='text-sm'>{modelNumber}</h3>
                        </div>
                        ...
                        {readmore && <div>
                            <div className='flex justify-between my-4 items-center capitalize'>
                                <h3>input voltage</h3>
                                <h3 className='text-sm'>{inputVoltage}</h3>
                            </div>
                            <div className='flex justify-between my-4 items-center capitalize'>
                                <h3>output voltage</h3>
                                <h3 className='text-sm'>{outputVoltage}</h3>
                            </div>
                            <div className='flex justify-between my-4 items-center capitalize'>
                                <h3>output plugsNumber</h3>
                                <h3 className='text-sm'>{outputPlugsNumber}</h3>
                            </div>
                            <div className='flex justify-between my-4 items-center capitalize'>
                                <h3>phase</h3>
                            ...
                            <div className='flex justify-between my-4 items-center capitalize'>
                                <h3>batteries number</h3>
                                <h3 className='text-sm'>{batteriesNumber}</h3>
                            </div>
                        </div>}
                    ...
                    <div className="lg:flex mt-5 lg:items-center justify-between space-y-3 lg:space-y-2">
                        ...
                        <button onClick={() => handelBuyClick(upsId)} className="text-white btn capitalize bg-orange-500 border-0 py-2 px-6 focus:outline-none hover:bg-orange-700 rounded">Buy Now</button>
                    </div>
                </div>
            </div>}
        </div>
