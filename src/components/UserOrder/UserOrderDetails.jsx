...
const UserOrderDetails = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      let { data, status } = await axios.get(
        `https://omepc-server.onrender.com/api/omEpc/buy/${orderId}`
      );
  ...
  const handelCancel = (id) => {
    fetch(`https://omepc-server.onrender.com/api/omEpc/buy/cancel/${id}`, {
      method: "PUT",
      headers: {
      ...
      <div className="max-w-7xl mx-auto mt-10 ">
        ...
        <div className="modal modal-bottom sm:modal-middle">
          <div className="modal-box">
            ...
            <p className="mt-5 text-xs lg:text-sm text-red-500 ">
            ...
            <div className="modal-action justify-start mt-3">
              <button
                ...
                className="btn btn-sm py-2 bg-red-500 rounded-none border-none capitalize hover:bg-red-700"
              >
                Cancel order
              </button>
              ...
              <label
                htmlFor="cancelOrder"
                className="btn btn-sm bg-white rounded-none border-blue-500 hover:text-white capitalize hover:bg-blue-700 text-black"
              >
                Don't cancel
              </label>
            </div>
        ...
        <div className="flex flex-col-reverse md:flex-col-reverse lg:flex-row justify-between lg:shadow-md shadow-sm px-10 py-28 bg-white">
          <div className="lg:mt-0 mt-0 md:mt-5">
            ...
            <p className="text-sm mt-5 ">
              {city}, {address}- {postCode}, West Bengal
            </p>
            <p className="text-lg font-semibold mt-5">
              {" "}
              Phone number{" "}
              <span className="text-sm font-normal">+91 {contact}</span>
            </p>
          </div>
            ...
            <div className="flex flex-col lg:flex-row md:flex-row md:ml-[-2rem]  lg:justify-between md:justify-start md:items-center lg:items-center lg:space-x-4 md:space-x-5">
              <div>
                <ul className="steps mt-5 text-xs">
                  <li
                    data-content={isCanceled ? "✓" : isApproved ? "✓" : "✕"}
                    className={`step ${
                      isCanceled
                        ? "step-error"
                        : isApproved
                  ...
                  <li
                    data-content={isCanceled ? "✓" : isShipped ? "✓" : "✕"}
                    className={`step ${
                      isCanceled
                        ? "step-error"
                        : isShipped
                  ...
                  <li
                    data-content={isCanceled ? "✓" : outOfDelivery ? "✓" : "✕"}
                    className={`step ${
                      isCanceled
                        ? "step-error"
                        : outOfDelivery
              ...
              <div className="mt-5 lg:mt-0 md:mt-2">
                {isCanceled ? (
                  ...
                  <label
                    ...
                    htmlFor={openModal ? "cancelOrder" : ""}
                    className={`flex items-center space-x-1 ${
                      outOfDelivery ? "btn-disabled" : ""
                    }`}
                  >
